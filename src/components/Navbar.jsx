...
export default function Navbar() {
  ...
  return (
    <nav 
      className="sticky top-0 z-50 w-full bg-[#0a0f0c]/95 border-b border-white/6 backdrop-blur-md overflow-hidden"
      role="navigation"
      aria-label="Main Navigation"
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 h-16 flex items-center justify-between gap-4">

        {/* ── Logo (SEO: Title Attribute) ── */}
        <Link to="/" 
        ...
        {/* ── Desktop links (SEO: Nav List structure) ── */}
          ...
          {links.map(({ label, to, title }) => {
            ...
            return (
              <li key={to}>
                <Link
                  to={to}
                  title={title} // Important for SEO crawlers
                  className={`
        ...
        <div className="hidden md:flex items-center gap-2.5">
          ...
          <Link
            to="/compare"
            title="Compare Top Nigerian Forex Brokers"
        ...
        {/* ── Mobile hamburger (Accessibility: Aria Expanded) ── */}
        <button
      ...
      >
        <div className="border-t border-white/6 px-4 py-4 flex flex-col gap-1 w-full box-border">
          ...
          <div className="flex items-center justify-between px-4 py-2 text-[0.7rem] text-zinc-500 w-full box-border uppercase tracking-widest font-mono">
          ...
          <div className="px-2">
            <Link
              to="/compare"
              onClick={() => setOpen(false)}
              className="mt-1 flex items-center justify-center gap-2 w-full py-3 text-sm font-semibold text-white bg-green-600 rounded-lg no-underline active:scale-[0.98] transition-transform"
            >
